...
//designCapacity mAh:设计容量（mAh），terminate Voltage:终止电压（mV），taperCurrent:减弱电流（mA）
bool bq27421_init( uint16_t designCapacity_mAh, uint16_t terminateVoltage_mV, uint16_t taperCurrent_mA )
{
    uint16_t designEnergy_mWh, taperRate, flags=0, checksumOld, checksumRead;
    uint8_t checksumNew;
    uint16_t timeout;
    ...
    // Seal gauge
 	//Seal电量计一方面可以防止正常运行时错误写入电量计参数，另一方面可以保证电量计参数不被第三方读出
...
bool i2c_write_test( uint16_t subcommand )
{
    HAL_StatusTypeDef ret;
    uint8_t i2c_data[2];

    i2c_data[0] = (uint8_t)(subcommand >> 8);
    i2c_data[1] = (uint8_t)(subcommand & 0x00FF);
    

    ret = HAL_I2C_Master_Transmit( &hi2c4, (uint16_t)BQ27421_I2C_ADDRESS, i2c_data, 2, 100/*HAL_BQ27421_TIMEOUT*/ );
    if( ret != HAL_OK )
    {
        return false;
    }
